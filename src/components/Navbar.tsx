...
export default function Navbar({ activeSection, onNavigate }: NavbarProps) {
  ...
  const navLinkClass = (section: SectionId) =>
    `nav-link text-[0.72rem] font-semibold transition-colors duration-300 tracking-[0.08em] uppercase ${
                ...
                <div className="mt-4 flex flex-col gap-2.5 sm:gap-3">
                  {navItems.map((item) => (
                    <button
                      ...
                      type="button"
                      onClick={() => navigate(item.section)}
                      className={`rounded-md border px-3.5 py-2.5 text-left text-[0.7rem] font-semibold uppercase tracking-[0.08em] transition-colors sm:px-4 sm:py-3 sm:text-xs ${
